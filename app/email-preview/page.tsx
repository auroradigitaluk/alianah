...
const EMAIL_TYPES = [
  { value: "donation", label: "Donation confirmation" },
  { value: "water-donation", label: "Water project donation" },
  { value: "sponsorship-donation", label: "Sponsorship donation" },
  { value: "sponsorship-completion", label: "Sponsorship completion" },
  { value: "water-completion", label: "Water project completion" },
  { value: "fundraiser-welcome", label: "Fundraiser welcome" },
  { value: "fundraiser-donation-notification", label: "Fundraiser donation notification" },
  { value: "fundraiser-otp", label: "Fundraiser OTP / login code" },
...
export default function EmailPreviewPage() {
  const [selectedType, setSelectedType] = React.useState<string>(EMAIL_TYPES[0].value)
  ...
  const donationEmail = React.useMemo(() => {
  ...
  const previewSrc = React.useMemo(() => {
    if (selectedType === "donation") {
      ...
      return `/email-preview/donation?${params.toString()}`
    }
    return `/email-preview/${selectedType}`
  }, [
    selectedType,
    donorName,
    orderNumber,
  ...
  const previewSubject =
    selectedType === "donation" ? donationEmail.subject : EMAIL_TYPES.find((t) => t.value === selectedType)?.label ?? "Preview"

  const showDonationForm = selectedType === "donation"
    ...
    <div className="p-4 sm:p-6 space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Preview how all outgoing emails will look. Select a template below.
        </p>
        <p className="text-xs text-muted-foreground mt-1">
          All templates use the same layout: logo, brand greens, and consistent styling.
        </p>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-5">
        {showDonationForm && (
          <Card className="lg:col-span-2">
            <CardHeader>
              ...
              <p className="text-xs text-muted-foreground">Edit sample data to update the preview.</p>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <Label htmlFor="giftAid" className="font-normal cursor-pointer">Gift Aid claimed</Label>
              </div>
              <div className="flex items-center gap-2">
                <Checkbox
        ...
        {!showDonationForm && (
          <Card className="lg:col-span-2">
            <CardHeader>
              <CardTitle>{EMAIL_TYPES.find((t) => t.value === selectedType)?.label ?? selectedType}</CardTitle>
              <p className="text-xs text-muted-foreground">Preview with sample data. This is how the email will look when sent.</p>
            </CardHeader>
            <CardContent />
          </Card>
        )}

        <Card className={showDonationForm ? "lg:col-span-3" : "lg:col-span-5"}>
          <CardHeader>
