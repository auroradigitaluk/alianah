...
import { SponsorshipCountryForm } from "@/components/sponsorship-country-form"
...
async function getCountry(id: string) {
  try {
    return await prisma.sponsorshipProjectCountry.findUnique({
      where: { id },
    })
  } catch (error) {
    return null
...
export default async function EditSponsorshipCountryPage({
  params,
}: {
          ...
          <div className="flex flex-col gap-4 py-4 md:gap-4 sm:gap-6 md:py-6">
            <div className="px-2 sm:px-4 lg:px-6">
              <div className="flex flex-col gap-4 sm:gap-6 max-w-2xl">
                ...
                <SponsorshipCountryForm country={country} />
              </div>
            </div>
          </div>
        </div>
