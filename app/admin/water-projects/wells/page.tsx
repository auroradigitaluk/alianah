...
async function getDonations(staffId: string | null) {
  try {
    const project = await prisma.waterProject.findUnique({
      where: { projectType: "WATER_WELL" },
      include: {
        donations: {
          // Staff can see all Water Well donations here (no staff filter).
          include: {
...
export default async function WaterWellsDonationsPage({
  searchParams,
}: {
  ...
  return (
    <>
      <AdminHeader title="Water Wells - Donations" />
            ...
            <div className="px-2 sm:px-4 lg:px-6">
              <div className="flex flex-col gap-4 sm:gap-6">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <h2 className="text-base sm:text-lg font-semibold">Water Wells Donations</h2>
                    <p className="text-xs sm:text-sm text-muted-foreground">Comprehensive management for all Water Well donations</p>
                  </div>
                  {staffUsers.length > 0 && (
                ...
                <div>
                  {donations.length === 0 ? (
                    ...
                    <WaterProjectDonationsTable donations={donations} projectType="WATER_WELL" initialOpenId={initialOpenId} />
                  )}
                </div>
              </div>
            </div>
