...
export function SponsorshipForm({ project, countries }: SponsorshipFormProps) {
  ...
  const handleAddCountries = async () => {
    ...
    try {
      ...
      const promises = countriesToAdd.map(countryData =>
        fetch("/api/admin/sponsorships/countries", {
          method: "POST",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = project ? `/api/admin/sponsorships/${project.id}` : "/api/admin/sponsorships"
